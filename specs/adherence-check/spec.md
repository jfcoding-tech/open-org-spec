...
- **Scope tree coverage.** Walk the repo's top-level directories. For each top-level directory that is a governed scope — identified by the presence of any of: `spec.md`, `README.md`, `governance/`, `people.md`, `feedback.md`, or `decisions/` — check whether it appears in the declared scope tree. A top-level directory absent from the scope tree emits a `gap` addressed to the manifest owner. Excludes: `.git/`, `.github/`, `.claude/`, `.open-org-spec/`, `open-org-spec/` (the standard submodule), and any path explicitly listed as excluded in the extension.
- **Self-containment check.** Any observability command file that reads from `open-org-spec/` or `.open-org-spec/extensions/` at runtime emits a `gap` at HIGH severity. Command files must be self-contained — all logic inline, extension points resolved at install time only.
- **Version sentinel check.** Any command file in the adopter's command directory whose `canonical_spec_version` does not match the current standard version emits a `gap` at MEDIUM severity. `/adhere-to tooling` should reinstall from the updated template.
- **Missing observability outputs.** If `observability` is `active` and `governance/observability/` is missing expected output files (such as `risk-load.md`, `owner-load.md`) that are older than 14 days, emit a `warning` at LOW severity.
...
- **Log entry matches `disposition_at`.** For each risk file where `disposition_at` is set and `status` is not in `(closed, resolved)`: check that the `## Log` section contains at least one `### <disposition_at date> —` heading (strict `YYYY-MM-DD` format). Missing entry emits a `violation` — a disposition date without a corresponding log entry means a decision was recorded without its reasoning.
